...
class ScoreModel(nn.Module):
    def __init__(
        ...
        # Kept out of the state dict (persistent=False). E13 arrived on a branch whose
        # checkpoints are not the ones this tree evaluates: making the buffer persistent
        # adds a key that every checkpoint trained before the merge -- cifar_ds3 and the
        # four A5 arms, i.e. all of stage A -- lacks, and a strict load of those fails.
        # Nothing is lost: z_center is a running statistic rebuilt from scratch on every
        # run (there is no resume path), EMA already excludes it, and with centring off
        # _prepare_z never reads it.
        self.register_buffer("z_center", torch.zeros(self.k), persistent=False)
